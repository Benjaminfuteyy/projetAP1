class projet extends Program {
    //variables globales
	String motATaper = "";
	boolean motSuivant = true;
	boolean commencerNouveauJeu = true;
	int indice = 0; //position dans le mot à taper
	class score {
		double resultat = 0.0;
		String nom = "";
	}
    ...
    //on utilisera des fichiers txt à la place
	...
	void afficherTableaux(String[] tableau){ //affichage d'un tableau passé en paramètre avec un mot par ligne
...
void testCreerTableau(){
	...
	String menus(){ //premier menu où l'on choisi entre jouer, afficher les tableaux, ou les meilleurs resultats
	...
	String[] selectionNiveau(){ //Deuxieme menu que l'utilisateur voit, selection du tableau utilisé pour tirer au hasard les mots
...
void testMotAuHasard(){
	...
	String motAuHasard(String[] tableauDeMots){ //tire un mot au hasard dans le tableau passé en paramètre
		return tableauDeMots[(int)(random() * length(tableauDeMots))];
	}

	void calculAffichageResultat(int nbMotsCorrects){ //calcul range et affiche le résultat de l'utilisateur
	//calcul du resultat et du rang de l'utilisateur par rapport aux parties précédentes
	//on demande à l'utilisateur de rentrer son nom
	//stockage
	//affichage
}

	String toString (score nouveauScore){ //affichage d'un .class
	return nouveauScore.nom + " a fait un resultat de " + nouveauScore.resultat;
}

void testVerification(){
	assertFalse(verification('a',"renard",2));
	assertFalse(verification(' ',"renard",1));
	assertTrue(verification('r',"renard",5));
}

	boolean verification(char c, String motHasard, int indice){ //verifie si le caractère passé en paramètre correspond au caractère du mot tiré au sort de même position
		return false;
	}

	void testmotBon(){
		assertTrue(motBon("renard", "renard"));
		assertFalse(motBon("renard", "renzrd"));
		assertFalse(motBon("renard",""));
		assertFalse(motBon("renard","ergheroghgierog"));

	}
	boolean motBon(String motHasard, String motTape){ //cette fonction test si le mot tapé correspond au mot tiré au sort une fois que l'utilisateur a fini de taper son mot (càd il a appuyé sur [Entrée])
	if(length(motHasard)==length(motTape)){
		if(equals(motHasard,motTape)){
...
void testProchainMot(){

}

	boolean prochainMot(char c){ //passe au mot suivant si '\n' (touche [Entrée]) est détecté
	if(c=='\n'){//OU le temps est écoulé
...
void testRetourArriere(){
	...
	boolean retourArriere(char c){ //on supprime le dernier caractère affiché/tapé, -1 sur l'indice
	return true;
}

	void keyTypedInConsole(char c){ //fonction eJava prenant en paramètre un caractère tapé au clavier
		//à utiliser dans cette fonction:
			//verification(char c, String mot, int indice)
			//motBon
			//prochainMot
		//note: il faudra faire gérer l'indice(+1,-1) en dehors des fonctions
		//changer les couleurs de texte, vider les lignes, etc.. (à gérer dans les fonctions, sans doute)
	}
	...
	void algorithm(){
		boolean temps = false; //a enlever quand on aura trouvé comment gérer un chronomètre
		while(commencerNouveauJeu){
			int nbMotsCorrects = 0;
			if(equals(menus(),"1")){
				String[] tableauChoisi = selectionNiveau();
				//clearScreen();
				while(temps){ //tant que le teps n'est pas écoulé
				motATaper = motAuHasard(tableauChoisi);
				motSuivant = true;
					enableKeyTypedInConsole(motSuivant); //appel la fonction keyTypedInConsole avec un nouveau mot
				}
			}
